...
- **Bug cutview /vedere — APERTO (8.3.4)**: rendering corrotto della metà tagliata in modalità sezione. **Diagnostica raffinata 2026-05-08**: NON dipende dall'opacità della mesh come ipotizzato in 8.3.3, ma dall'**angolo camera-piano di taglio**. In certe orientazioni (probabilmente camera quasi parallela al piano) la mesh appare piatta arancione; in altre orientazioni a opacità 100% il rendering è corretto. La patch 8.3.3 (forzare `material.transparent = true` nello slider opacità di [syntesis-icp-vedere.html:7917-7928](backend/static/syntesis-icp-vedere.html:7917)) è stata mantenuta in produzione perché non rompe nulla, ma da sola non risolve. Filoni da investigare a freddo nella prossima sessione, in ordine di sospetto:
    1. **PlaneGeometry della cap troppo grande**: in [buildCapForSection riga 6535-6537](backend/static/syntesis-icp-vedere.html:6535) `capSize = max(bbox.size.length() * 2, 50)` — quando la camera è quasi parallela al piano di taglio, il cap (ruotato dal `setFromUnitVectors(z, s.normal)`) può intersecare il near plane della camera o "avvolgerla", coprendo tutto lo schermo come un piano arancione gigante. Diagnosi: aggiungere `capMesh.frustumCulled = false` non aiuta; servirebbe limitare `capSize` o spostare il cap rispetto al `s.center` lungo la normale per evitare l'inversione.
    2. **Stencil buffer compromesso a camera tangente**: con camera quasi parallela al piano, le facce front/back delle stencil meshes degenerano in linee → conteggi increment/decrement sbagliati, cap colorato dove non dovrebbe.
    3. **Z-fighting cap vs mesh**: l'offset `position.addScaledVector(s.normal, -0.001)` ([riga 6561](backend/static/syntesis-icp-vedere.html:6561)) può non bastare quando le due superfici sono quasi alla stessa profondità dal punto di vista della camera. Provare `polygonOffset` sulla cap, o offset relativo a `bbox.size.length()` invece che fisso a 0.001 mm.
  Riproduzione: caricare un STL su `/vedere`, attivare la sezione, ruotare la camera fino a renderla quasi parallela al piano di taglio (orbit verso una vista grazing). A opacità 100% la mesh appare piatta arancione in quegli angoli. A opacità < 100% il bug si maschera (la trasparenza nasconde sia la cap difettosa sia la mesh dietro). Note: 8.3.3 ha cambiato il flag transparent dello slider; il bug originale non era causato (solo) da quello, l'ipotesi era incompleta.
