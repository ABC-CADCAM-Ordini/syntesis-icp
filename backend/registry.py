...
#
# History:
#   8.6.7          (2026-06-02): FIX scalino transparency-sorting scansione (syntesis-analyzer-v3b.html). 8.6.6 (depthWrite accoppiato all'opacita') toglieva l'occlusione ma lasciava uno scalino brusco a 95-100%: la scansione (mesh grande/avvolgente, transparent) col transparent pass sort per-oggetto (sortObjects:true default) flippava l'ordine rispetto ai MUA trasparenti -> blending instabile sui posteriori ad alta opacita'. Fix: scanMesh.renderOrder=1 (creazione in loadScanFile r.2512) -> scansione sempre DOPO i MUA (0) e SOTTO gli overlay (spline asse-medio 5, frecce/pivot 9998+) -> ordine deterministico, scalino eliminato. COMPLEMENTARE al depthWrite 8.6.6 (tenuto: depthWrite:false <100% evita che la scansione scriva depth e occluda posteriori/spline; renderOrder fissa l'ordine di blending). NON toccati: scanbodyScanMesh (restano 0), camera/renderer/side (Fix A FrontSide intatto), MUA. Un solo punto. Bump v3b <title>+ANALIZZA_BUILD 8.6.6 -> 8.6.7. docs/MAPPA_FUNZIONALE.md versione mappata 8.6.7.
#   8.6.6          (2026-06-02): FIX occlusione scansione in Solid (syntesis-analyzer-v3b.html): la scansione, transparent ma con depthWrite:true, occludeva per depth-test i MUA dietro (sparivano in Solid; in Wireframe no). Causa: il path opacita' dell'Albero Scena (treeUnified_setScanOpacity r.3754) impostava transparent+opacity ma NON depthWrite -> restava true a qualsiasi %. Fix: accoppiato mat.depthWrite=(opacity>=1.0) sulla scansione in 3 punti: treeUnified_setScanOpacity (slider; v gia' normalizzato 0-1), loadScanFile (al load: opacity 0.85 -> depthWrite:false), treeUnified_ghostAll (ghost, solo blocco scansione). Pattern gia' presente in /vedere (7925) e in applyOpacityToThree (18015, NON toccato). NON toccati: camera/renderer, side (Fix A resta FrontSide), MUA, Scene Registry. Niente logarithmicDepthBuffer (Fix B in panchina), niente renderOrder. A opacity 100% la scansione torna a occludere (atteso). Bump v3b <title>+ANALIZZA_BUILD 8.6.5 -> 8.6.6. docs/MAPPA_FUNZIONALE.md: versione mappata 8.6.6.
#   8.6.5          (2026-06-02): FIX A rendering viewport principale /analizzare (syntesis-analyzer-v3b.html): back-face culling sui MUA. I 3 materiali dei componenti MUA posati - scanbody (r.2784), matematica (r.2788), analogo (r.2792) - erano side:THREE.DoubleSide -> niente culling -> la cavita' interna (foro canale-vite) si vedeva dall'esterno. Portati a side:THREE.FrontSide. Scansione (scanMesh r.2512) lasciata DoubleSide; camera/renderer NON toccati. Fix B (z-fighting depth sui MUA vicini, leva logarithmicDepthBuffer) NON incluso, separato. Verifica winding a video post-deploy: se un MUA appare bucato al contrario / sparisce -> normali CAD invertite, rivalutare computeVertexNormals()/flip indici (in quel caso stop, non forzare). Tocca v3b: bump <title> + window.ANALIZZA_BUILD 8.5.0 -> 8.6.5 + _DATE 2026-06-02. docs/MAPPA_FUNZIONALE.md: versione mappata -> 8.6.5. Niente CACHEBUST.
