...
> **Versione software mappata:** 8.22.0 — **Data:** 2026-06-09
...
**Piazzamento — SEEDING a 3 punti (8.20.0, slice 2b-1.2, sostituisce il `click_center`):** `replacePlaceFromSeed` calcola la posa con `kabsch(markerPts_locali, scanPts_mondo)` (**riuso del `kabsch()` di Misurare/ICP**) → `Matrix4` (`world = R·local + t`) → `decompose` → `group.position/quaternion`. I 3 punti del seme nascono da: (a) **anteprima interattiva** — secondo `THREE.WebGLRenderer` su `#replacePreviewCanvas` (`replacePreviewInit/Render/Dispose`, camera orbit col drag, mesh marker dalla cache offsettata col baricentro all'origine → `worldToLocal` sui click = coord nel frame **locale canonico**), `replacePreviewPickAt` raccoglie 3 punti (dot colorati 1 rosso/2 verde/3 blu, **figli della mesh** così seguono l'orbita); (b) **3 punti gemelli sulla scansione** — `replaceOnViewportClick` in fase `pickScan` raccoglie 3 punti mondo (dot colorati corrispondenti, riuso `replaceMakeDot`; hover dot 2b-1.1 come cursore). Macchina a stati `replaceSeed {phase: idle|pickMarker|pickScan, markerPts, scanPts, markerDots, scanDots}` + contatore `#replaceSeedCounter` + **Annulla punto** (`replaceSeedUndo`) + **Ricomincia** (`replaceSeedClear`). Per-marker memorizzati `seedMarkerPts`/`seedScanPts`/`seedR`/`seedT` (**seme per 2b-2**, l'ICP/Raffina). Fetch marker via `replaceFetchMarkerGeo` (`GET /api/rit/markers/{sha}` → ArrayBuffer → `sostParseSTLToGeometry`, cache). Reset `_hardResetReplace` (dispone anche l'anteprima). Stato `replace*` separato da `sost*`. **Match/ICP fuori scope** (= slice 2b-2).

> **UX guida del piazzamento (8.19.0 — slice 2b-1.1):** punto rosso del riferimento (`click_center`) + guida. `replaceMakeDot(point,opts)` clona l'idioma `ensurePivotMarker` (sfera rossa `0xFF3B30` r~0.45mm, `MeshBasicMaterial` `depthTest:false`, persistente). **Dot del riferimento** (`replaceMakeDot`): **in 8.20.0** sono i **3 punti del seme** sulla scansione (`dotMeshes[]` nel record), rimossi+disposti in `replaceClearScene`/`_hardResetReplace` (il single-dot del `click_center` 2b-1.1 è superato dal seeding 3-punti). **Hover dot live**: `replaceOnViewportHover` agganciato a init come listener **unico e passivo** (`{passive:true}`, gemello dell'attach del `click` a [~2570]), **gated** internamente a `currentWorkflow==='replace' && replaceSeed.phase==='pickScan' && replaceMesh` → raycast su `replaceMesh`, sposta `replaceHoverDot` (semi-trasparente); `replaceHideHoverDot` su nessun-hit/click/uscita/reset. **Isolamento**: nessun `preventDefault`/`stopPropagation` → il controller camera custom ([2418-2445], `mousemove` solo durante drag) e `onViewportClick` restano identici; early-return in ogni altro workflow. **Guida**: div statico `#replaceGuide` nel `#panelReplace` + messaggio `#replaceStatus` in fase di placement.
