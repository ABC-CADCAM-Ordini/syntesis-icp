...
> **Versione software mappata:** 8.60.0 — **Data:** 2026-06-14
...
| `.replace-only` | 7 (3 in 8.18-8.20; +3 in 8.21.0; +1 in 8.59.0) | [~5064](../backend/static/syntesis-analyzer-v3b.html#L5064) gestione centralizzata a fine `selectWorkflow` | nascondi `none` | nascondi `none` | nascondi `none` | nascondi `none` (replace → mostra `''`) | **8.18.0**: gemello di `.sostituire-only` (`querySelectorAll('.replace-only')` + display per `wf === 'replace'`). Marca i bottoni di `#panelReplace`: "+ Posiziona (3 punti)" (`#replaceBtnPlace`), "Annulla punto" (`#replaceSeedUndo`), "Ricomincia" (`#replaceSeedReset`); **8.21.0**: "Avanti / nuovo" (`#replaceBtnNext`), "Concludi · Esporta" (`#replaceBtnFinish`, **8.46.0 funzionale** → `replaceExportSTL`); **8.22.0**: "Raffina" (`#replaceBtnRefine`) ora **funzionale** → `replaceRefineAll` (ICP per-marker); **8.59.0**: "Raffina+" (`#replaceBtnRefineSrv`, verde) → `replaceRefineServer` (ICP point-to-plane SERVER full-res, opzione parallela). |
