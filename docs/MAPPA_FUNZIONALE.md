...
> **Versione software mappata:** 8.27.0 — **Data:** 2026-06-10
...
| Albero scena | `#layerTree`/`#layersPanel` (sx) | checkbox/color/range/✕ | `replaceRebuildTree` (8.26.0) | **2b-3a**: scansione (visibilità/colore/opacità) + per-marker (visibilità/colore/RMSD/elimina), come negli altri workflow; gemello `sostRebuildTree`, target colore `replacescan`/`replace:`+num | `replaceRebuildTree` / `replaceToggleScanVisibility` / `replaceSetScanOpacity` / `replaceTogglePlacedVisibility` |
