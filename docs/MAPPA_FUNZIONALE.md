...
> **Versione software mappata:** 8.41.0 — **Data:** 2026-06-11
...
> **UX guida del piazzamento (8.19.0 — slice 2b-1.1, evoluta a 8.23.0):** `replaceMakeDot` clona l'idioma `ensurePivotMarker` (sfera rossa `depthTest:false`); **dal 8.23.0 serve solo per l'hover dot** (cursore live), mentre i **3 punti del seme** (anteprima + scansione) sono **sprite numerati 1/2/3** (`_replaceNumSprite`, `CanvasTexture` billboard colorato), ceduti al marker piazzato in `dotMeshes[]` alla conferma e rimossi+disposti (con texture) via `_replaceDisposeDot` in `replaceClearScene`/`_hardResetReplace`/`replaceDeletePlaced`. **Hover dot live**: `replaceOnViewportHover` agganciato a init come listener **unico e passivo** (`{passive:true}`, gemello dell'attach del `click` a [~2570]), **gated** internamente a `currentWorkflow==='replace' && replaceSeed.phase==='pickScan' && replaceMesh` → raycast su `replaceMesh`, sposta `replaceHoverDot` (semi-trasparente); `replaceHideHoverDot` su nessun-hit/click/uscita/reset. **Isolamento**: nessun `preventDefault`/`stopPropagation` → il controller camera custom ([2418-2445], `mousemove` solo durante drag) e `onViewportClick` restano identici; early-return in ogni altro workflow. **Guida**: div statico `#replaceGuide` nel `#panelReplace` + messaggio `#replaceStatus` in fase di placement.
