...
> **Versione software mappata:** 8.24.0 — **Data:** 2026-06-10
...
| Flusso: Raffina / Avanti·nuovo / Concludi | `#replaceFlow` → `#replaceBtnRefine` / `#replaceBtnNext` / `#replaceBtnFinish` `.replace-only` | click | `replaceRefineAll` (Raffina) / `replaceNextScanbody` (Avanti) | mostrato con ≥1 marker. **Raffina (8.22.0, FUNZIONALE)**: ICP per-marker `_replaceDoRefine` (campiona marker nel mondo + pesi facce-piatte ×5 → crop cilindrico scan → ICP pesato+outlier Tukey+convergenza+idempotenza → delta rigido al group; riuso `kabsch`); abilitato con ≥1 marker; RMSD mostrato in lista. **Avanti/nuovo**: azzera type+seme+anteprima per il prossimo scanbody. **Concludi** (`#replaceBtnFinish`): **scaffolding disabilitato** → 2b-3 (vista finale + export + albero file). | [~2017-2021](../backend/static/syntesis-analyzer-v3b.html#L2017) markup; `_replaceDoRefine`/`replaceRefineAll` |
