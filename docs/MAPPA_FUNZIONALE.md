...
> **Versione software mappata:** 8.32.0 — **Data:** 2026-06-10
...
| Vista Sorgente/Sostituto + origine (pending) | `#replaceViewRow` → `#replaceViewSrc`/`#replaceViewSub`/`#replaceViewOrigin` (finestra guida) | click / change | `replaceSetPendingView` / `replaceTogglePendingOrigin` | **8.32.0** ispezione accoppiamento: a fase `posed` mostra il toggle Sorgente↔Sostituto + checkbox origine x0/y0/z0 sul marker pending (`replacePending.viewMode`/`showOrigin` → `_replaceApplyView`); gating/sync in `replaceSeedUpdateUI`. Default vista SORGENTE allineato + origine ON. | markup riga `#replaceViewRow` (finestra guida); `replaceSetPendingView`/`replaceTogglePendingOrigin`/`_replaceApplyView` |
