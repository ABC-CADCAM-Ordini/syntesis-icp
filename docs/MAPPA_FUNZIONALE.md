...
> **Versione software mappata:** 8.53.0 — **Data:** 2026-06-12
...
| Label 2D | `.replace-label` (`.divergence-label`) | loop `animate` | `replaceEnsureLabelElements` / `replaceUpdateLabels` | **8.53.0**: etichetta = **identità libreria** `#N Marca Modello Ømm` (`_replaceLibId`, da `p.marca/modello/diametro` congelati alla posa); **grafica/posizione come i MUA in Analizza** — `replaceUpdateLabels` clone di `updateDivergenceLabels` (pillola `.divergence-label` colore impianto `p.color`, offset asse×10mm, **linea guida SVG** su `#labelLines` + compensazione `body.zoom`); rimosso lo sfondo scuro forzato `#0D1B2A`. Prima (≤8.52.0): solo numeri `#N` proiettati | loop `animate` |
