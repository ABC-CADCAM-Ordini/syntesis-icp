...
> **Versione software mappata:** 8.35.0 — **Data:** 2026-06-10
...
| + Posiziona (3 punti) | `#replaceBtnPlace` `.replace-only` | click | `replaceStartPlacement` → `replacePreviewPickAt` (fase `pickMarker`, anteprima) → `replaceOnViewportClick` (fase `pickScan`, scansione) → `replacePlaceFromSeed` | avvia il seeding 3+3 punti e piazza il marker via `kabsch` (posa grezza, no match/ICP) | [1996](../backend/static/syntesis-analyzer-v3b.html#L1996) markup; [15876](../backend/static/syntesis-analyzer-v3b.html#L15876)/[15888](../backend/static/syntesis-analyzer-v3b.html#L15888)/[15916](../backend/static/syntesis-analyzer-v3b.html#L15916) |
...
**⚑ FLUSSO ATTIVO (8.35.0): UNICO a 3 PUNTI.** L'auto-ICP è **rimossa** (scelta utente: "togliamo l'auto, gestiamo solo una cosa"): `#replaceBtnAlign` ("▶ Allinea (3 punti)") chiama `replaceStartThreePoint`. Flusso: **+ Nuovo impianto → clicca scanbody → ▶ Allinea (3 punti) → 3 punti sul marker + 3 sulla scansione (`replacePlaceFromSeed`) → ✓ Conferma**. `replacePlaceFromSeed` costruisce MADRE (sorgente, `meshSrc` verde, children[1]) + FIGLIO (sostituto, `meshSub`, children[0]) + terna origine (children[2]) → madre+figlio in scena (pending) e albero (confermati). `replaceAutoPlaceFromSource` + `_replaceEstimateCadRadius` + gate RMSD (8.34.0) = **dead code annotato** (non cablati; ri-abilitabili ricablando il pulsante). _Sotto, la storia dell'auto-ICP 8.31–8.34 (disattivata):_
