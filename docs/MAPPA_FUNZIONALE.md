...
> **Versione software mappata:** 8.21.0 — **Data:** 2026-06-09
...
| Flusso: Raffina / Avanti·nuovo / Concludi | `#replaceFlow` → `#replaceBtnRefine` / `#replaceBtnNext` / `#replaceBtnFinish` `.replace-only` | click | `replaceNextScanbody` (solo Avanti) | **8.21.0**: mostrato con ≥1 marker. **Avanti/nuovo**: azzera type+seme+anteprima per il prossimo scanbody (i piazzati restano). **Raffina** (→2b-2) e **Concludi** (→2b-3) sono **scaffolding disabilitato** (tooltip) per mostrare il flusso target posiziona→raffina→avanti→concludi | [~2017-2021](../backend/static/syntesis-analyzer-v3b.html#L2017) markup |
